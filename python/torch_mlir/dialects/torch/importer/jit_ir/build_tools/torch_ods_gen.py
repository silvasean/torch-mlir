...
def raw_emit_op(operator: JitOperator,
    ...
    p_cpp = lambda *args: emitter_cpp.print(*args)
    ...
    def generic_result_name(i): return "result" + \
        (str(i) if multiple_results else "")
...
def emit_ops(emitter_td: TextEmitter, emitter_cpp: TextEmitter, registry: Registry):
    def emit(key, **kwargs):
        emit_op(registry[key], emitter_td, emitter_cpp, **kwargs)

    def emit_with_mutating_variants(key, **kwargs):
        operator = registry[key]
        emit_op(operator, emitter_td, emitter_cpp, **kwargs)
        ns, unqual, overload = operator.triple
        emit_op(registry.get_by_triple((ns, unqual + "_", overload)),
                emitter_td, emitter_cpp,
...
def main(args: argparse.Namespace):
    ...
    cpp_path = os.path.join(args.torch_ir_lib_dir, "GeneratedTorchOps.cpp")
    with open(td_path, "w") as f_td, open(cpp_path, "w") as f_cpp:
        emitter_td = TextEmitter(f_td)
        emitter_cpp = TextEmitter(f_cpp)
        emitter_td.print(ODS_BANNER)
        emitter_cpp.print(CPP_BANNER)
        emit_ops(emitter_td, emitter_cpp, registry)
...
def _create_argparse() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--torch_ir_lib_dir",
        required=True,
        help="Directory in lib/ containing the Torch dialect definition")
    parser.add_argument(
        "--debug_registry_dump",
